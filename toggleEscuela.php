...
try {
    ...
    if (!$r || $r->num_rows === 0) throw new Exception('Escuela no encontrada');
    $row = $r->fetch_assoc();
    $nuevo = $row['estado'] ? 0 : 1;
    $conn->query("UPDATE escuelas SET estado = $nuevo WHERE id = $id");
    echo json_encode(['success' => true, 'estado' => $nuevo, 'message' => $nuevo ? 'Escuela activada' : 'Escuela desactivada']);
    $conn->close();
} catch (Throwable $e) {
